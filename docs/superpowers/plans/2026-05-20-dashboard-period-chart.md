...
```bash
cd /home/tim/projects/runscribe/backend && python3 -m pytest tests/test_builder.py::test_bucket_by_day_last_7_days_uses_weekday_labels tests/test_builder.py::test_bucket_by_day_month_uses_day_of_month_labels -v
```
...
```bash
cd /home/tim/projects/runscribe/backend && python3 -m pytest tests/test_builder.py::test_bucket_by_week_sun_start_2026 -v
```
...
```bash
cd /home/tim/projects/runscribe/backend && python3 -m pytest tests/test_builder.py::test_compute_period_data_last_7_days_sum_matches_summary tests/test_builder.py::test_compute_period_data_month_uses_calendar_boundaries tests/test_builder.py::test_compute_period_data_year_53_weeks_2026 -v
```
...
```bash
cd /home/tim/projects/runscribe/backend && python3 -m pytest tests/test_builder.py -v -k "period_data or bucket_by"
```
...
```bash
cd /home/tim/projects/runscribe/backend && python3 -m pytest tests/test_builder.py::test_rebuild_globals_writes_volume_field -v
```
...
```bash
cd /home/tim/projects/runscribe/backend && python3 -m pytest tests/test_builder.py -v
```
...
```bash
cd /home/tim/projects/runscribe/backend && python3 -m pytest -v
```
...
```bash
cd /home/tim/projects/runscribe/frontend && npx tsc --noEmit -p tsconfig.json
```
...
```bash
cd /home/tim/projects/runscribe/backend && setsid nohup ../.venv/bin/python -m uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload > /tmp/runscribe-backend.log 2>&1 < /dev/null &
disown
```
...
```bash
sleep 3 && python3 -c "import json; d=json.load(open('/home/tim/projects/runscribe/data-dev/static/dashboard.json')); print('summary keys:', list(d.get('summary',{}).keys())); print('volume keys:', list(d.get('volume',{}).keys())); print('month buckets:', len(d['volume']['month']['buckets'])); print('year buckets:', len(d['volume']['year']['buckets']))"
```
...
```bash
python3 -c "import json; d=json.load(open('/home/tim/projects/runscribe/data-dev/static/dashboard.json'));
for p in ('last_7_days','month','year'):
...
```bash
pgrep -f "vite" >/dev/null || (cd /home/tim/projects/runscribe/frontend && setsid nohup npm run dev -- --host > /tmp/runscribe-vite.log 2>&1 < /dev/null & disown)
```
...
- [ ] **Step 3: Note any unexpected behavior in `/tmp/runscribe-verify.log`** (no actual command needed — just record findings before committing)
...
```bash
cd /home/tim/projects/runscribe && git status
```
...
```bash
cd /home/tim/projects/runscribe && docker build -f Dockerfile -t ricetim/runscribe:latest .
```
...
```bash
docker push ricetim/runscribe:latest
```
...
```bash
ssh coruscant 'cd ~/.docker_config/runscribe && docker compose pull && docker compose down && docker compose up -d'
```
...
Open `http://runscribe.timothyrice.org/` and run through the same checklist as Task 10 Step 2.
...
```bash
# On coruscant
docker tag ricetim/runscribe:prev ricetim/runscribe:latest
