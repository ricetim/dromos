...
1. `sqlite3 /home/tim/.docker_config/runscribe/runscribe.db "SELECT activity_id, COUNT(*) FROM activityshoe GROUP BY activity_id HAVING COUNT(*) > 1"` — should return zero rows.
2. Open https://runscribe.timothyrice.org/activities/119 — detail view shows the *current* shoe (FWD VIA 2).
