...
If you see `PermissionError: [Errno 13] Permission denied: '/data'`: that's a pre-existing environment issue when `app.main` is imported outside Docker. The `tiles.py` import tries to `mkdir /data`. Set `DATA_DIR=/tmp/dromos-test python3 -m pytest …` to work around it locally.
