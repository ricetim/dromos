...
```bash
cd /home/tim/projects/dromos/frontend && npm run build 2>&1 | grep -E "^.*error TS" | head -20
```
...
```bash
cd /home/tim/projects/dromos && git add frontend/src/pages/ActivityDetail.tsx && git commit -m "feat: arrow key navigation between activities on detail page
...
```bash
cd /home/tim/projects/dromos && docker compose build
```
...
```bash
sudo docker compose pull && sudo docker compose up -d dromos
```
